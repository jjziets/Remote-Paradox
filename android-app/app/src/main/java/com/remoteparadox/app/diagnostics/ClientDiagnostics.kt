...
class DiagnosticsApplication : Application() {
    override fun onCreate() {
        super.onCreate()
        Diagnostics.initialize(File(noBackupFilesDir, "diagnostics"), "phone", BuildConfig.VERSION_NAME, BuildConfig.VERSION_CODE.toLong())
        TokenStore(this)
        ClientDiagnostics.record(DiagnosticEvent(kind = "app_start", source = "phone_app"))
        PhoneDiagnosticReports.initialize(this)
    }
}
